...
func (h *Harness) runCriterionCommands(
	...
	slog.Debug("trace", "cmd", harnessLogCmd, "operation", "agent.harness.Harness.runCriterionCommands",
	...
	for _, it := range snap.criteria {
		...
		dir := commandEvidenceDir(h.opts.ReportDir, cycleID, it.ID)
		if err := os.MkdirAll(dir, 0o700); err != nil {
			return nil, fmt.Errorf("mkdir command evidence %s: %w", dir, err)
		}
		for _, cmd := range it.VerifyCommands {
			seq := cmd.SortOrder
			base := commandArtifactBase(h.opts.ReportDir, cycleID, it.ID, seq)
			stdoutPath := base + ".stdout"
			stderrPath := base + ".stderr"
			metaPath := base + ".meta.json"

			cmdCtx, cancel := context.WithTimeout(parentCtx, timeout)
			started := h.opts.Clock()
			stdout, stderr, exitCode, runErr := execFn(cmdCtx, h.opts.WorkingDir, cmd.Command)
			cancel()
			durationMS := h.opts.Clock().Sub(started).Milliseconds()
	...
	if len(persist) > 0 {
		if err := h.store.UpsertCommandRuns(parentCtx, cycleID, attemptSeq, persist); err != nil {
			slog.Warn("agent harness UpsertCommandRuns failed",
				"cmd", harnessLogCmd, "operation", "agent.harness.Harness.runCriterionCommands.upsert_err",
