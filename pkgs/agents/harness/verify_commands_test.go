...
func TestRunCriterionCommands_writesEvidenceAndPromptSection(t *testing.T) {
	...
	reportDir := t.TempDir()
	workDir := t.TempDir()
	h := New(st, runnerfake.New(), Options{WorkingDir: workDir, ReportDir: reportDir})
	...
	evidence, err := h.runCriterionCommands(ctx, cycleID, 1, snap, selfReport, func(ctx context.Context, dir, command string) ([]byte, []byte, int, error) {
