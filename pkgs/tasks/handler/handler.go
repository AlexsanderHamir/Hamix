...
func NewHandler(s *store.Store, hub *SSEHub, rep *repo.Root, opts ...HandlerOption) http.Handler {
	...
	m.Handle("GET /projects/{id}/goals", http.HandlerFunc(h.listProjectGoals))
	m.Handle("POST /projects/{id}/goals", http.HandlerFunc(h.createProjectGoal))
	m.Handle("GET /projects/{id}/goals/{goalId}", http.HandlerFunc(h.getProjectGoal))
	m.Handle("PATCH /projects/{id}/goals/{goalId}", http.HandlerFunc(h.patchProjectGoal))
	m.Handle("DELETE /projects/{id}/goals/{goalId}", http.HandlerFunc(h.deleteProjectGoal))
