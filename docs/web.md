...
Expanded cycle rows in `TaskCyclesPanel` load `GET /tasks/{id}/cycles/{cycleId}/verdicts`. When the worker indexed git commits for the cycle, the panel shows a repo → branch breadcrumb and commit rows (`git_context`, `commits[]`) with **status badges** (`Eligible`, `Observed`, …) above the per-criterion verdict list. Badge tooltips and the section legend explain verify eligibility without reading domain docs — see [ADR-0016](./adr/ADR-0016-observe-vs-admit-commits.md).

The task detail page also loads **`GET /tasks/{id}/commits`** via `TaskCommitsPanel` / `useTaskCommits` — task-wide commit history deduped by SHA, refetched on `task_cycle_changed` SSE. The commits section includes the same eligibility legend. Clicking a commit row navigates to **`/tasks/{id}/commits/{sha}`** (`TaskCommitDiffPage`), which loads **`GET /repo/diff?sha=`** with GitHub-style summary stats, syntax-highlighted hunks (refractor + `react-diff-view`), unified/split toggle, file navigator, and collapsible large files. Copy helpers: `web/src/tasks/components/task-detail/commits/commitDisplay.ts`. Parsers: `web/src/api/parseTaskApiCycles.ts`; types: `web/src/types/cycle.ts`. See [domain/cycle-commits.md](./domain/cycle-commits.md) and [commit-eligibility.md](./domain/commit-eligibility.md).
